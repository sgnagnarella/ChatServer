...
public class FriendServlet extends HttpServlet {
  ...
  throws ServletException, IOException {
    ...
    Iterator<String> friendList = friendStore.getFriends(user).iterator();
    
    Set<String> friensdWhoCanSeeUser = friendStore.getFriendsWhoCanSeeUser(user);
    ...
    //TODO Seba : We should advice about the login only to the users that can see this user.
    while(friendList.hasNext()){
  	  String friend = friendList.next() ;
      if(!friend.equals(user)){
        outputTxt +="<friend><name>" + friend +"</name></friend>\n";
        if(friensdWhoCanSeeUser.contains(friend)){
